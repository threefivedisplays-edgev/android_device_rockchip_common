...
PARAMETER_WITH_GAPPS=`get_build_var PARAMETER_WITH_GAPPS`
...
echo PARAMETER_WITH_GAPPS==$PARAMETER_WITH_GAPPS
...
then
			...
			if [ ! $PARAMETER_WITH_GAPPS ]; then
				cp -a $PARAMETER $IMAGE_PATH/parameter.txt
			else
				cp -a $PARAMETER_WITH_GAPPS $IMAGE_PATH/parameter.txt
			fi
	        echo "done."
	      fi
else
